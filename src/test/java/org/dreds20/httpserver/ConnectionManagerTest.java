...
public class ConnectionManagerTest {
    ...
    void httpVerbNotSpecifiedOnReturnedPage() throws Exception {
        ...
        assertThat(HttpResponse.from(connectionManager.getResponse(request)).statusCode()).isEqualTo(HttpStatus.METHOD_NOT_ALLOWED);
    }
    ...
    void successfulResponseSentStatusCodeIsCorrect() throws Exception {
        ...
        assertThat(HttpResponse.from(connectionManager.getResponse(request)).statusCode()).isEqualTo(HttpStatus.OK);
    }
    ...
    void successfulResponseSentContentTypeIsCorrect() throws Exception {
        ...
        setupPageMock(HttpVerb.GET, path);
        when(contentLoaderMock.getContent(Paths.get(path).toAbsolutePath().toUri())).thenReturn(body);
        assertThat(HttpResponse.from(connectionManager.getResponse(request)).headers().get("Content-Type"))
                .isEqualTo("text/html");
    }
    ...
    void successfulResponseSentBodyIsCorrect() throws Exception {
        ...
        setupPageMock(HttpVerb.GET, path);
        when(contentLoaderMock.getContent(Paths.get(path).toAbsolutePath().toUri())).thenReturn(body);
        assertThat(HttpResponse.from(connectionManager.getResponse(request)).body())
                .isEqualTo(body);
    }
}
